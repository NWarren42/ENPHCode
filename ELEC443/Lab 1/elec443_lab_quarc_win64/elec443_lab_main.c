...
typedef RT_MODEL_elec443_lab_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * elec443_lab(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(elec443_lab_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(elec443_lab_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://elec443_lab:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: elec443_lab -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(elec443_lab_DW.HILReadEncoderTimebase_Task,
        (t_clock) elec443_lab_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
