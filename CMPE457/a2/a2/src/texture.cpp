...

{
  ...
  // Copy image to our own texmap, flipping in y because image has
  ...
  for (unsigned int y=0; y<height; y++)
	...
	texmap[ c + 4*x + 4*width*(height-y-1) ] = image[i++];
...
// Get luminance from r,g,b pixel

float Texture::toLuminance( Pixel &p )
...
void Texture::draw( vec2 lowerLeft, vec2 upperRight, vec2 texLL, vec2 texUR )

{
  ...
  vec2 texcoords[4] = { vec2( texLL.x, texUR.y ), 
			vec2( texLL.x, texLL.y ), 
			vec2( texUR.x, texUR.y ), 
			vec2( texUR.x, texLL.y ) };
